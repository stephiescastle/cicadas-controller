/**************************************
  ...
  - main board intensity max or high knob
  - main board intensity low knob
  - reset button
  - lifecycles mode switch (off sets motors to low intensity)
...
// BROOD Values
//long defaultTimeScale = 1000;
long defaultTimeScale = 1120;
//long defaultTimeScale = 1270;
unsigned long defaultSleepTime = 2; // 10
unsigned long defaultAwakeTime = 16; // 24
unsigned long defaultRampBasis = 6000; // 6000
bool awake = true;
int strongBroodRate = 8;

// TODO replicate vars for 3 groups.
...
// knobPin[1] = main board intensity roof (previously time scale)
...
int motorSpeed(bool varied = false) {
  if(awake) {
    intensity = map(knobValue[0], 0, 1023, 0, intensityHigh);
...
long variedIntensity() {
  if (!strongBrood()) {
    ...
    return random(constrain(intensityHigh-40, intensityMin, intensityHigh), intensityHigh+1);
  }
}
...
void setup() {
  ...
  intensityHigh = map(knobValue[1], 0, 1023, 0, 255);
  intensityLow = map(knobValue[0], 0, 1023, 0, 255);
  ...
  Serial.println(rampBasis);
  Serial.print("SETUP buttonCounter: ");
  Serial.println(buttonCounter);
...
void loop() {
  ...
  if (switchValue[1] == 0) {
    ...
    for (int i = 0; i < motorCount; i++) {
      // use low pwm setting
      motor[i].ramp(map(knobValue[0], 0, 1023, 0, intensityLow), 0);
    }
  } else {
    // LIFE CYCLES MODE

    // toggle between waking up and going to sleep
